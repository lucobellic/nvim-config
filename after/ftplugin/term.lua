...
-- Disable horizontal scrolling when cursor is at the end of the line
-- sidescrolloff is window-local, sidescroll is global
vim.wo.sidescrolloff = 0

-- Ensure these options persist even if window/buffer changes
local group = vim.api.nvim_create_augroup('TermScrollOptions', { clear = false })
vim.api.nvim_create_autocmd({ 'BufWinEnter', 'WinEnter' }, {
  group = group,
  buffer = 0,
  callback = function() vim.wo.sidescrolloff = 0 end,
  desc = 'Disable horizontal scroll offset in terminal windows',
})
